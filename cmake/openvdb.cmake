...
function(openvdb_import_target)
    macro(openvdb_push_variable var value)
    ...
    macro(openvdb_pop_variable var)
        ...
        elseif(OPENVDB_OLD_${var}_TYPE STREQUAL NONE_TYPE)
            ...
            message(WARNING "OPENVDB_OLD_${var}_TYPE: ${OPENVDB_OLD_${var}_TYPE}")
            message(FATAL_ERROR "Trying to pop a variable that has not been pushed: ${var}")
        endif()
    endmacro()

    macro(openvdb_ignore_package NAME)
        set(OPENVDB_DUMMY_DIR "${CMAKE_CURRENT_BINARY_DIR}/openvdb_cmake/${NAME}")
        file(WRITE ${OPENVDB_DUMMY_DIR}/${NAME}Config.cmake "")
        openvdb_push_variable(${NAME}_DIR ${OPENVDB_DUMMY_DIR})
        openvdb_push_variable(${NAME}_ROOT ${OPENVDB_DUMMY_DIR})
    endmacro()

    macro(openvdb_unignore_package NAME)
        openvdb_pop_variable(${NAME}_DIR)
        openvdb_pop_variable(${NAME}_ROOT)
    endmacro()
    ...
    openvdb_ignore_package(TBB)
    openvdb_ignore_package(Boost)
    openvdb_ignore_package(IlmBase)
    include(onetbb)
    include(boost)
    include(ilmbase)
    ...
    openvdb_unignore_package(TBB)
    openvdb_unignore_package(Boost)
    openvdb_unignore_package(IlmBase)
